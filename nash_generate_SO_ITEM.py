...
ofcsv=open('generated_SO_ITEM.csv','w')
ofsql=open('generated_SO_ITEM.sql','w')
i=base
while i < last:
        salesorderid=str(i).zfill(10)
        soitem=random.randint(1,10)
        if(soitem+i) <= last:
                somax=soitem
                j=1
                while j <= somax:
                        salesorderitem=str(j*10)
                        ...
                        csvstr=salesorderid+";"+salesorderitem+";"+product_productid+";"+currency+";"+str(grossamount)+";"+noteid+";"+netamount+";"+taxamount+";"+itematpstatus+";"+opitempos+";"+str(quantity)+";"+quantityunit+";"+deliverydate+"\n"
                        sqlstr="insert into "+sqlstring+" VALUES ('" +salesorderid+"','"+salesorderitem+"','"+product_productid+"','"+noteid+"','"+currency+"','"+str(grossamount)+"','"+netamount+"','"+taxamount+"','"+itematpstatus+"','"+opitempos+"','"+str(quantity)+"','"+quantityunit+"','"+deliverydate+"');\n"
                        ofcsv.write(csvstr)
                        ofsql.write(sqlstr)
#                        print csvstr
#                        print sqlstr
